...
class QlindaCartPoleEnv(gymnasium.Env):
    ...
    def step(self, action):    
        ...
        #print(self.pos_delta_model.predict(np.array([self.state]), verbose=0))
        #print(f"new_pos_delta:{new_pos_delta}, new_theta_delta:{new_theta_delta}")
        ...
        state = np.array([new_pos, new_theta, new_pos_delta, new_theta_delta])
        ...
        return state, reward, terminated, truncated, info, done

    def reset(self, pos_offset=0.):
        self.state = np.array(STATE_MEAN)
        self.state[0::5] = self.state[0::5] + pos_offset
        state = np.append(self.state[-5:-3], self.state[-3:-1]) #You can consider using the last one state, not the first one
        return state
